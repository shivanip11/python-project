...
from modules.admin_ops import list_bikes, add_bike, update_bike, remove_bike, view_rentals, change_rental_status, generate_basic_reports
...
def admin_menu():
    while True:
        ...
        if choice == '1':
            model = input('Model: ')
            typ = input('Type: ')
            hr = float(input('Hourly rate: '))
            dy = float(input('Daily rate: '))
            b = add_bike(model, typ, hr, dy)
            print('\nAdded:', b)
            pause()
        elif choice == '2':
            bid = input('bike_id: ')
            ...
            if typ: updates['type'] = typ
            if hr: updates['rent_hourly'] = float(hr)
            if dy: updates['rent_daily'] = float(dy)
            ...
            p = get_pricing()
            print('\nCurrent pricing:', p)
            hh = input('New hourly rate (blank keep): ')
            dd = input('New daily rate (blank keep): ')
            if hh or dd:
                hhv = float(hh) if hh else p['base_rate_hourly']
                ddv = float(dd) if dd else p['base_rate_daily']
                update_pricing(hhv, ddv)
                print('Pricing updated')
            pause()
        elif choice == '7':
            r = generate_basic_reports()
            print('\nGenerated report:')
            print(r)
            pause()
        elif choice == '8':
